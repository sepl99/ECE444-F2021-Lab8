...
class ScoringService(object):
    ...
    def get_models(cls,language):
        ...
        if cls.model_en == None: # and language.lower() =="en":
            cls.model_en = ft.load_model(os.path.join(model_path, 'model-en.ftz'))
        if cls.model_es == None and language.lower() =="es":
            cls.model_es = ft.load_model(os.path.join(model_path, 'model-es.ftz'))
        if cls.model_fr == None and language.lower() =="fr":
            cls.model_fr = ft.load_model(os.path.join(model_path, 'model-fr.ftz'))
        if cls.model_it == None and language.lower() =="it":
            cls.model_it = ft.load_model(os.path.join(model_path, 'model-it.ftz'))
        if cls.model_de == None and language.lower() =="de":
            cls.model_de = ft.load_model(os.path.join(model_path, 'model-de.ftz')) #ft.load_model('/tmp/model-de.ftz')
